#!/usr/bin/env python3
"""
NOKK Audio Generator - All 4 Languages
Mature male voices, louder/projected feel
"""
...
# Mature male voices
# Bill - Wise, Mature, Balanced (works well for all languages with eleven_turbo_v2_5)
# Using Brian for deeper/louder alternative
VOICES = {
    'en': 'pqHfZKP75CvOlQylNhV4',  # Bill - Wise, Mature
    'ko': 'nPczCjzI2devNBz1zQrb',  # Brian - Deep, Resonant (good for Korean with multilingual model)
    'ja': 'JBFqnCBsd6RMkjVDRZzb',  # George - Warm, Captivating (good for Japanese)
    'es': 'onwK4e9ZLuTAKqWW03F9',  # Daniel - Steady Broadcaster (good for Spanish)
}

# Tone settings - louder/projected feel with higher similarity_boost
TONES = {
    'normal': {'stability': 0.4, 'similarity_boost': 0.85, 'speed': 1.1},
    'firm': {'stability': 0.6, 'similarity_boost': 0.9, 'speed': 1.0},
    'angry': {'stability': 0.25, 'similarity_boost': 0.95, 'speed': 1.15},
}
...
def main():
    print("=" * 60)
    print("🎙️  NOKK Audio Generator - ALL 4 LANGUAGES")
    print("    Mature male voices, louder/projected feel")
    print("=" * 60)
    ...
    for lang, phrases in PHRASES.items():
        for phrase_id, text in phrases.items():
            for tone in TONES.keys():
                tasks.append((lang, phrase_id, text, tone))
    
    total_chars = sum(len(text) for _, _, text, _ in tasks)
    print(f"\n📊 Total Files: {len(tasks)}")
    print(f"📊 Total Characters: {total_chars}")
    print(f"📊 Languages: EN, KO, JA, ES")
    print("=" * 60 + "\n")
    
    success, fail = 0, 0
    
    for i, (lang, phrase_id, text, tone) in enumerate(tasks, 1):
        ...
        if ok:
            ...
            print(f"❌ [{i}/{len(tasks)}] {filename}: {err[:80] if err else 'Unknown'}")
            fail += 1
